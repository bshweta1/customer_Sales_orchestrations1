...
# # --------------------------------------------json------------------------
#
#     raw_path = f"s3://{BUCKET_NAME}/Raw_data/stores.json"
...
#     staging_path = f"s3://{BUCKET_NAME}/staging_data/stores"
#     df.write.mode("overwrite").parquet(staging_path)
#     raw_path = f"s3a://{BUCKET_NAME}/Raw_data/stores.json"
...
#     df.write.mode("overwrite").parquet(staging_path)
#
#     print(f"stores staged successfully at {staging_path}")
#
#
#
...
def ingest_stores(spark=None):
    """
    Ingest stores data from S3 raw JSON and write to staging parquet.
    Args:
        spark (SparkSession, optional): Spark session. Creates one if None.
    """
    if spark is None:
        spark = SparkSession.builder.appName("Ingest_stores").getOrCreate()
