...
# ------------------------------
spark=get_spark(app_name="dim_customers_etl")
stg_path , dim_path = get_paths(table_name="customers")
...
stgdf=stgdf.dropDuplicates(["CUSTOMER_ID","EMAIL_ADDRESS"])
stgdf = (
    stgdf
...
if dim_exists:
    dim_table.alias("target").merge(
        ...
        condition="target.EMAIL_ADDRESS <> source.EMAIL_ADDRESS OR target.FULL_NAME <> source.FULL_NAME",
